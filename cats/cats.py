...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(paragraph):
        cleaned_paragraph = split(remove_punctuation(paragraph))
        for item1 in topic:
            for item2 in cleaned_paragraph:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    list_diffs = [diff_function(typed_word, element, limit) for element in word_list]
    for x in word_list:
        if x == typed_word:
            return typed_word
    if min(list_diffs) > limit:
        return typed_word
    else:
        return word_list[list_diffs.index(min(list_diffs))] 
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    def helper(typed, source, limit):
        ...
        if typed == source:
            return limit
        if typed[0] != source[0]:
            return helper(typed[1:], source[1:], limit-1)
        else:
            return helper(typed[1:], source[1:], limit)
    difference = abs(len(source)-len(typed))
    changes = helper(typed,source,limit-difference)
    return limit - changes
    # END PROBLEM 6
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    lst = []
    for mini_lst in times_per_player:
        lst_inside = []
        for x in range(len(mini_lst)-1):
            lst_inside.append(mini_lst[x+1] - mini_lst[x])
...
def fastest_words(match):
    ...
    for i in word_indices:
        times = [player[i] for player in get_all_times(match)]
        index = times.index(min(times))
        lst[index] += [get_word(match, i)]
    return lst
    # END PROBLEM 10
